...
const Month = ({id, selects, callback}) => {
  ...
  const creditChange = (id, val) => {
    ...
    selects[id].firstQuantity = val;
    callback(selects.firstQuantity, val);
  };
  ...
  const setFieldState = (index, value) => {
    ...
    selects[index].month = value;
    callback(selects.month, value);
  };
  ...
  return (
    <View style={styles.tableContainer}>
      ...
      <TextInput
        style={styles.input}
        onChangeText={val => setFieldState(id, Number(val))}
        keyboardType="number-pad"
      />
    </View>
  );
